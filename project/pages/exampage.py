...
class State(rx.State):
    ...
    async def fetch_assessment_id(self):
        ...
        url = f"http://localhost:8000/get_assessment_id/{self.subject_id}/{self.exam_name}"
        try:
            response = await asyncio.to_thread(requests.get, url)
    ...
    async def get_detail(self, name: str):
        self.exam_name = name
        print(f"subject_id: {self.subject_id}, exam_name: {self.exam_name}")
    ...
    async def get_file(self):
        url = f"http://localhost:8000/get_file_exam/{self.subject_id}/{self.exam_name}"
        try:
            response = await asyncio.to_thread(requests.get, url)
            ...
            if response.status_code == 200:
                ...
                yield rx.redirect("/exam_details")
            else:
    ...
    async def handle_upload_submit(self, files: list[rx.UploadFile]):
        ...
        # API URL
        url = "http://127.0.0.1:8000/submit_exam/"
...
def exam_item(name: str, exam_date: str) -> rx.Component:
    # status_colors = {
    #     "upcoming": "#EFFAFF",
    #     "completed": "#D3F8E2",
    # }
    ...
    return rx.button(
                ...
                shadow="md",
                _hover={"bg": "#BFD9E5"},
                on_click=State.get_detail(name), 
)
...
def exam() -> rx.Component:
    return rx.vstack(
        rx.hstack(
            rx.box(
                rx.vstack(
                    rx.text("Exam", size="4", weight="bold"),
                    rx.foreach(
                        State.exam_list.items(),
                        lambda item: exam_item(item[0],item[1][0]),
                    ),
